    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col border border-slate-700">
        <div className="p-4 border-b border-slate-700 flex justify-between items-center">
          ...
          <button onClick={onCancel} className="text-slate-400 hover:text-slate-200"><X size={24} /></button>
        </div>
        <div className="p-6 flex flex-col items-center gap-4">
          <div 
            ref={containerRef}
            className="w-64 h-64 bg-slate-900 rounded-xl overflow-hidden relative cursor-move touch-none border-2 border-slate-700 shadow-inner"
          ...
          <div className="w-full flex items-center gap-3 text-slate-400">
             <ZoomOut size={16} />
             <input type="range" min="0.1" max="3" step="0.05" value={scale} onChange={(e) => setScale(parseFloat(e.target.value))} className="flex-1 h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer" />
             <ZoomIn size={16} />
          </div>
          <p className="text-center text-xs text-slate-400">Drag to move • Pinch/Slider to zoom</p>
        </div>
        <div className="p-4 border-t border-slate-700 flex gap-3">
          <button onClick={onCancel} className="flex-1 py-2.5 font-bold text-slate-300 bg-slate-700 rounded-xl hover:bg-slate-600">Cancel</button>
          <button onClick={handleSave} className="flex-1 py-2.5 font-bold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow-md">Save</button>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-all duration-300">
      <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all duration-300 scale-100 max-h-[90vh] overflow-y-auto animate-in zoom-in-95 fade-in border border-slate-700">
        <div className="px-6 py-4 border-b border-slate-700 flex justify-between items-center bg-slate-800 sticky top-0 z-10">
          <h3 className="font-bold text-lg text-white tracking-tight">{title}</h3>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200 hover:bg-slate-700 p-1 rounded-full transition-colors"><XCircle size={24} /></button>
        </div>
...
export default function StudentDatabaseApp() {
  ...
  const markAttendance = async (status) => {
    ...
    try {
      ...
      setIsAttendanceModalOpen(false); // Auto-close logic
          ...
          <div className="w-full text-center">
            ...
            {isMbk ? (
               ...
               <>
                 ...
                 {!isLulus && <div className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wide mb-2 shadow-sm ${getSubjectBadgeColor(student.subject)}`}>{student.subject}</div>}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="flex items-center gap-3">
              <div className="bg-indigo-600 p-2.5 rounded-xl"><GraduationCap className="text-white h-6 w-6" /></div>
             ...
             {selectedStudentForProgress ? (
               <div className="rounded-3xl border shadow-sm overflow-hidden bg-slate-800 border-slate-700">
                 ...
                 <div className="border-b p-6 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-900/50 border-slate-700">
                    ...
                    <button onClick={() => setSelectedStudentForProgress(null)} className="px-4 py-2 border rounded-xl text-sm font-bold shadow-sm bg-slate-800 border-slate-600 text-slate-200 hover:bg-slate-700">
                      Change Student
                    </button>
                 </div>
               ...
               <div className="rounded-3xl border shadow-sm p-6 md:p-12 text-center bg-slate-800 border-slate-700">
                  <div className="max-w-2xl mx-auto">
                    <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 bg-indigo-900/50">
                      <TrendingUp className="text-indigo-400 w-8 h-8" />
                    </div>
                    <h2 className="text-2xl font-bold mb-2 text-white">Student Progress Tracker</h2>
                    <p className="mb-8 text-slate-400">Select a student from the list below to view or update their mastery of skills (Kemahiran).</p>
                    
                    {/* Filters */}
                    <div className="flex flex-col sm:flex-row gap-3 w-full mb-4">
                        <div className="relative group flex-1">
                          <select 
                            className="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500 font-bold text-sm appearance-none bg-slate-700 border-slate-600 text-white" 
                    ...
                    <div className="relative mb-6">
                      ...
                      <input 
                        ...
                        className="w-full pl-12 pr-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium bg-slate-700 border-slate-600 text-white placeholder-slate-400"
                        value={searchQuery}
                    ...
                    <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                       {filteredStudents.map(student => (
                         <button 
                           ...
                           className="w-full flex items-center gap-3 p-3 border rounded-xl transition-all text-left group bg-slate-800 border-slate-700 hover:border-indigo-500"
                         >
