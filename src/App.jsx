...
export default function StudentDatabaseApp() {
  ...
  const markAttendance = async (status) => {
    ...
    try {
      ...
      setIsAttendanceModalOpen(false); // Auto-close logic applied here
  ...
  const renderStudentCard = (student, sectionType) => {
    ...
    // Check if this card is currently selected by admin (touchscreen optimize)
    ...
    return (
      <div 
        ...
        className={`bg-white dark:bg-slate-800 rounded-2xl shadow-sm hover:shadow-lg border transition-all duration-300 ${isSelected ? 'border-indigo-500 ring-2 ring-indigo-500 scale-[1.02]' : 'border-slate-200 dark:border-slate-700 hover:-translate-y-1'} group relative overflow-hidden flex flex-col cursor-pointer`}
      >
        ...
        <div className={`hidden sm:flex flex-col items-center gap-4 ${isMbk ? 'p-6' : 'p-4'}`}>
          ...
          {isMbk && (
            <div className="mt-2 w-full pt-4 border-t border-slate-100 dark:border-slate-700 flex flex-col gap-2">
              ...
              {student.mbkType === 'OKU' && student.qrCodeUrl && (
                <button onClick={(e) => { e.stopPropagation(); setFullScreenImage(student.qrCodeUrl); }} className="flex-1 flex items-center justify-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 py-2.5 rounded-xl transition-all shadow-sm"><QrCode size={16} /> Show QR</button>
              )}
            </div>
          )}
          ...
          {role === 'admin' && (
            <div className={`absolute top-2 right-2 flex ${isMbk ? 'flex-row' : 'flex-col'} gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 dark:bg-slate-800/90 p-1 rounded-lg backdrop-blur-sm shadow-sm border border-slate-100 dark:border-slate-700`}>
               ...
               {!isMbk && <button onClick={(e) => { e.stopPropagation(); toggleStudentStatus(student); }} className="p-1.5 text-purple-500 hover:bg-purple-50 dark:hover:bg-slate-700 rounded transition-colors" title="Change Status"><RotateCcw size={14} /></button>}
               <button onClick={(e) => { e.stopPropagation(); confirmDelete(student); }} className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-slate-700 rounded transition-colors" title="Delete"><Trash2 size={14} /></button>
            </div>
          )}
        </div>
        ...
        <div className="sm:hidden flex flex-row items-start p-3 gap-3 relative z-10">
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            {role === 'admin' && isSelected && (
              <div className="grid grid-cols-2 gap-1 w-[70px]">
                 ...
                 {isProfile && <button onClick={(e) => { e.stopPropagation(); openAttendanceModal(student); }} className="p-1 text-blue-500 bg-blue-50 dark:bg-slate-700 dark:border-slate-600 rounded border border-blue-100 flex justify-center"><Calendar size={12} /></button>}
                 <button onClick={(e) => { e.stopPropagation(); openEdit(student); }} className={`p-1 text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-700 border border-slate-100 dark:border-slate-600 flex justify-center rounded ${isMbk || isLulus ? '' : 'col-span-2'}`}><Edit2 size={12} /></button>
                 {!isMbk && <button onClick={(e) => { e.stopPropagation(); toggleStudentStatus(student); }} className="p-1 text-purple-500 bg-purple-50 dark:bg-slate-700 dark:border-slate-600 rounded border border-purple-100 flex justify-center"><RotateCcw size={12} /></button>}
                 <button onClick={(e) => { e.stopPropagation(); confirmDelete(student); }} className="p-1 text-red-500 bg-red-50 dark:bg-slate-700 dark:border-slate-600 rounded border border-red-100 flex justify-center col-span-2"><Trash2 size={12} /></button>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 min-w-0 text-left">
            ...
            {isProfile && (
              <div className="flex flex-col gap-1 w-full mt-1">
                <div className="flex justify-between items-center text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase"><span>Attendance</span><span className={stats.percent >= 75 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}>{stats.percent}%</span></div>
                <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden"><div className={`h-full rounded-full ${stats.percent >= 75 ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${stats.percent}%` }}></div></div>
              </div>
            )}

            {isMbk && student.remarks && <div className="text-[10px] text-slate-500 dark:text-slate-400 italic bg-yellow-50 dark:bg-yellow-900/20 px-2 py-1 rounded border border-yellow-100 dark:border-yellow-800/50 flex items-start gap-1 mt-1"><MessageSquare size={10} className="mt-0.5 flex-shrink-0 text-yellow-500" /><span className="line-clamp-2">{student.remarks}</span></div>}
            
            {isMbk && (
              <div className="mt-2 flex flex-col gap-1">
                 <button onClick={(e) => { e.stopPropagation(); window.open(student.docLink, '_blank'); }} disabled={!student.docLink} className={`flex items-center justify-center gap-1 text-[10px] font-bold py-1 px-2 rounded border ${student.docLink ? 'bg-indigo-50 text-indigo-600 border-indigo-100 dark:bg-indigo-900/30 dark:text-indigo-400 dark:border-indigo-800' : 'bg-slate-50 text-slate-400 border-slate-100 dark:bg-slate-800 dark:border-slate-700 cursor-not-allowed'}`}><FileText size={12} /> {student.docLink ? 'Docs' : 'No Docs'}</button>
                 {student.mbkType === 'OKU' && student.qrCodeUrl && (
                   <button onClick={(e) => { e.stopPropagation(); setFullScreenImage(student.qrCodeUrl); }} className="flex items-center justify-center gap-1 text-[10px] font-bold py-1 px-2 rounded border bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800"><QrCode size={12} /> QR Code</button>
                 )}
              </div>
            )}
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 font-sans selection:bg-indigo-200 dark:selection:bg-indigo-900 ${isDarkMode ? 'dark bg-slate-900 text-slate-200' : 'bg-gradient-to-br from-slate-50 to-slate-100 text-slate-800'}`}>
      ...
      {/* Top Navbar */}
      <nav className={`backdrop-blur-md border-b sticky top-0 z-30 transition-colors duration-300 ${isDarkMode ? 'bg-slate-900/90 border-slate-800' : 'bg-white/90 border-slate-200'}`}>
            ...
            <div className={`p-6 rounded-2xl border shadow-sm transition-colors ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="relative">
                  ...
                  <select 
                    className={`w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500 appearance-none font-medium transition-colors ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`}
                      ...
                      {filteredStudents.map(student => (
                        <tr key={student.id} className={`transition-colors ${isDarkMode ? 'hover:bg-slate-700/50' : 'hover:bg-slate-50/80'}`}>
                          ...
                          <td className="px-6 py-4">
                            <span className={`rounded px-2 py-1 font-mono text-xs ${isDarkMode ? 'bg-slate-700 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>
                              {student.className}
                            </span>
                          </td>
                 ...
                 <div className={`border-b p-6 flex flex-col md:flex-row justify-between items-center gap-4 transition-colors ${isDarkMode ? 'bg-slate-900/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className={`inline-block px-2 py-0.5 rounded text-xs font-bold shadow-sm ${getSubjectBadgeColor(selectedStudentForProgress.subject)}`}>
                  ...
                  <div className="max-w-2xl mx-auto">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full mb-4">
                        <div className="relative group flex-1">
                          <select 
                            className={`w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500 font-bold text-sm appearance-none transition-colors ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} 
           ...
           <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
             ...
             <div className={`flex flex-col lg:flex-row gap-4 mb-8 justify-between items-start lg:items-center p-4 rounded-2xl shadow-sm border transition-colors ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
                ...
                {currentSection === 'profile' && (
                  <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                    <div className="relative group">
                      <select className={`w-full sm:w-48 px-4 py-2.5 border rounded-xl font-bold text-sm appearance-none transition-colors focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={profileYearFilter} onChange={(e) => setProfileYearFilter(e.target.value)}>
                        <option value="All">Filter: All Years</option>
                        {availableYears.filter(y=>y!=='All').map(y => <option key={y} value={y}>{`Tahun ${y}`}</option>)}
                      </select>
                      <ChevronDown className="absolute right-3 top-3 text-slate-400 w-4 h-4 pointer-events-none" />
                    ...
                    <div className="relative group">
                      <select className={`w-full sm:w-48 px-4 py-2.5 border rounded-xl font-bold text-sm appearance-none transition-colors focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={classFilter} onChange={(e) => setClassFilter(e.target.value)}>
                        <option value="All">Filter: All Classes</option>
                        {availableClasses.filter(c=>c!=='All').map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                      <ChevronDown className="absolute right-3 top-3 text-slate-400 w-4 h-4 pointer-events-none" />
                    ...
                    <div className="relative group">
                      <select className={`w-full sm:w-48 px-4 py-2.5 border rounded-xl font-bold text-sm appearance-none transition-colors focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={subjectFilter} onChange={(e) => setSubjectFilter(e.target.value)}>
                        <option value="All">Semua Subjek</option>
                        {subjects.map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                      <ChevronDown className="absolute right-3 top-3 text-slate-400 w-4 h-4 pointer-events-none" />
                ...
                {currentSection === 'mbk' && (
                  <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center w-full lg:w-auto">
                    ...
                    <div className="relative group w-full sm:w-auto">
                      <select 
                        className={`w-full sm:w-48 px-4 py-2.5 border rounded-xl font-bold text-sm appearance-none transition-colors focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`}
                        value={mbkTypeFilter} 
                ...
                <div className="flex gap-3 w-full lg:w-auto items-center ml-auto">
                   {(role === 'admin' && (currentSection === 'profile' || currentSection === 'mbk')) && (
                     <button onClick={openAdd} className="flex-1 lg:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 text-white px-5 py-2.5 rounded-xl shadow-md font-bold text-sm">
                       <Plus size={18} strokeWidth={2.5}/> Add Student
                     </button>
                   )}
                   <button onClick={exportToExcel} className={`flex-1 lg:flex-none flex items-center justify-center gap-2 text-sm font-bold px-5 py-2.5 border rounded-xl transition-colors ${isDarkMode ? 'bg-slate-800 border-slate-600 text-slate-300 hover:bg-slate-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
             ...
             {loading ? (
               <div className="text-center py-24">
                 ...
                 <p className={isDarkMode ? 'text-slate-400' : 'text-slate-500'}>Loading database...</p>
               </div>
               ...
               <div className={`text-center py-24 rounded-3xl border border-dashed shadow-sm ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300'}`}>
                 ...
                 <h3 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>No students found</h3>
               </div>
             ) : (
               <div className="space-y-10">
                  ...
                  {currentSection === 'profile' && Object.keys(groupedProfileStudents).sort().map(className => {
                    ...
                    return (
                      <div key={className} className={`rounded-3xl border shadow-sm overflow-hidden ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                        <div className={`px-8 py-4 border-b ${style.border} flex justify-between ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
                          <h3 className={`font-extrabold ${style.text} text-lg flex items-center gap-3`}>
                            <School className={style.icon} size={20}/> {className}
                          </h3>
                          <span className={`text-xs font-bold ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-100'} ${style.icon} px-3 py-1.5 border rounded-lg shadow-sm`}>
                  ...
                  {currentSection === 'plan' && Object.keys(groupedPlanStudents).sort().map(yearGrp => (
                    <div key={yearGrp} className={`rounded-3xl border shadow-sm overflow-hidden ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                      <div className={`px-8 py-4 border-b flex justify-between ${isDarkMode ? 'bg-blue-900/30 border-blue-800' : 'bg-blue-50 border-blue-100'}`}>
                        <h3 className={`font-extrabold text-lg flex items-center gap-2 ${isDarkMode ? 'text-blue-400' : 'text-blue-900'}`}>
                          <BookOpenCheck className="text-blue-500" size={20}/>{yearGrp}
                        </h3>
                        <span className={`text-xs font-bold px-3 py-1.5 border rounded-lg shadow-sm ${isDarkMode ? 'bg-slate-900 border-blue-800 text-blue-400' : 'bg-white border-blue-100 text-blue-700'}`}>
                  ...
                  {currentSection === 'lulus' && Object.keys(groupedLulusStudents).sort().map(yearGrp => (
                    <div key={yearGrp} className={`rounded-3xl border shadow-sm overflow-hidden ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                      <div className={`px-8 py-4 border-b flex justify-between ${isDarkMode ? 'bg-purple-900/30 border-purple-800' : 'bg-purple-50 border-purple-100'}`}>
                        ...
                        <span className={`text-xs font-bold px-3 py-1.5 border rounded-lg shadow-sm ${isDarkMode ? 'bg-slate-900 border-purple-800 text-purple-400' : 'bg-white border-purple-100 text-purple-700'}`}>
      ...
      {/* --- MOBILE BOTTOM NAV --- */}
      <div className={`fixed bottom-0 left-0 w-full backdrop-blur-md border-t flex justify-around items-center z-50 sm:hidden shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] transition-colors duration-300 pb-safe ${isDarkMode ? 'bg-slate-900/90 border-slate-800' : 'bg-white/90 border-slate-200'}`}>
         {[
      ...
      <Modal isOpen={showAdminLogin} onClose={()=>{setShowAdminLogin(false); setLoginError('');}} title="Admin Login">
        <div className="flex flex-col items-center justify-center mb-6">
          <div className={`p-3 rounded-full mb-3 ${isDarkMode ? 'bg-indigo-900/30' : 'bg-blue-50'}`}>
            <Lock className={`w-8 h-8 ${isDarkMode ? 'text-indigo-400' : 'text-blue-600'}`} />
          </div>
          <p className={`text-sm text-center ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Enter admin password.</p>
        </div>
        <form onSubmit={handleAdminLogin} className="space-y-4">
          <input 
            ...
            autoFocus 
            className={`w-full p-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200'}`} 
            value={adminPassword} 
      ...
      <Modal isOpen={deleteConfirmation.isOpen} onClose={()=>setDeleteConfirmation({isOpen:false})} title="Confirm Deletion">
        <div className="flex flex-col items-center justify-center mb-6 text-center">
          ...
          <p className={isDarkMode ? 'text-slate-400' : 'text-gray-500'}>
            Are you sure you want to delete <span className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{deleteConfirmation.studentName}</span>? This action cannot be undone.
          </p>
        </div>
        <div className="flex gap-3">
          <button onClick={()=>setDeleteConfirmation({isOpen:false})} className={`flex-1 px-4 py-2.5 text-sm font-bold rounded-xl transition-colors ${isDarkMode ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>Cancel</button>
          ...
          {moveConfirmation.newStatus === 'Lulus' && (
            <div className={`w-full text-left p-3 rounded-lg border ${isDarkMode ? 'bg-slate-700 border-slate-600' : 'bg-gray-50 border-gray-200'}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <input type="date" className={`bg-transparent border-none focus:ring-0 text-sm font-medium w-full p-0 style-color-scheme-dark ${isDarkMode ? 'text-white' : 'text-gray-900'}`} value={moveDate} onChange={(e) => setMoveDate(e.target.value)} />
              </div>
            </div>
          )}
        </div>
        <div className="flex gap-3">
          <button onClick={()=>setMoveConfirmation({isOpen:false})} className={`flex-1 px-4 py-2.5 text-sm font-bold rounded-xl transition-colors ${isDarkMode ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>Cancel</button>
      ...
      <Modal isOpen={isNotesModalOpen} onClose={()=>setIsNotesModalOpen(false)} title="Catatan Murid">
        <div className="space-y-6">
          <div className={`flex items-center gap-4 p-4 rounded-xl ${isDarkMode ? 'bg-amber-900/20 border border-amber-800/50' : 'bg-amber-50'}`}>
          ...
          <form onSubmit={saveNote} className="space-y-3">
            <div>
              ...
              <input type="date" required className={`w-full px-3 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 style-color-scheme-dark ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-gray-300'}`} value={noteForm.date} onChange={e=>setNoteForm(p=>({...p, date:e.target.value}))}/>
            </div>
            <div>
              <label className={`block text-xs font-semibold uppercase mb-1 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Catatan (Note)</label>
              <textarea required rows="3" placeholder="Enter note details here..." className={`w-full px-3 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white placeholder-slate-400' : 'bg-white border-gray-300'}`} value={noteForm.text} onChange={e=>setNoteForm(p=>({...p, text:e.target.value}))}/>
            </div>
                ...
                <div key={n.id} className={`p-3 border rounded-xl text-sm group relative transition-colors ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50 border-gray-200'}`}>
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <div className={`flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-lg shadow-sm ${isDarkMode ? 'bg-slate-900/90' : 'bg-white/90'}`}>
                      <button type="button" onClick={()=>startEditNote(n)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded"><Edit2 size={14} className="text-blue-500"/></button>
                      <button type="button" onClick={()=>deleteNote(n.id)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded"><Trash2 size={14} className="text-red-500"/></button>
                    </div>
                  </div>
                  <p className={`text-sm mt-1 whitespace-pre-wrap ${isDarkMode ? 'text-slate-300' : 'text-gray-800'}`}>{n.text}</p>
                </div>
      ...
      <Modal isOpen={isAttendanceModalOpen} onClose={()=>setIsAttendanceModalOpen(false)} title="Manage Attendance">
         <div className="space-y-6">
           <div className={`flex items-center gap-4 p-4 rounded-xl border border-transparent ${isDarkMode ? 'bg-slate-800 dark:border-slate-700' : 'bg-gray-50'}`}>
           ...
           <div>
             ...
             <div className="flex gap-2">
               <input type="date" className={`flex-1 px-3 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 style-color-scheme-dark ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-gray-300'}`} value={attendanceDate} onChange={e=>setAttendanceDate(e.target.value)}/>
               <button onClick={()=>markAttendance('present')} className={`flex items-center justify-center gap-1 px-4 py-2 font-bold rounded-xl transition-colors ${isDarkMode ? 'bg-emerald-900/50 text-emerald-400 hover:bg-emerald-800/50' : 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'}`}><Check size={16}/> Present</button>
               <button onClick={()=>markAttendance('absent')} className={`flex items-center justify-center gap-1 px-4 py-2 font-bold rounded-xl transition-colors ${isDarkMode ? 'bg-red-900/50 text-red-400 hover:bg-red-800/50' : 'bg-red-100 text-red-700 hover:bg-red-200'}`}><X size={16}/> Absent</button>
             </div>
           </div>
             ...
             <div className={`max-h-48 overflow-y-auto border rounded-xl divide-y ${isDarkMode ? 'border-slate-700 divide-slate-700' : 'border-gray-100 divide-gray-100'}`}>
                {selectedStudentForAttendance?.attendanceRecords?.length > 0 ? (
                  ...
                  <div key={i} className={`py-3 px-4 flex justify-between items-center transition-colors ${isDarkMode ? 'hover:bg-slate-800' : 'hover:bg-gray-50'}`}>
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded uppercase ${r.status==='present' ? (isDarkMode?'bg-emerald-900/50 text-emerald-400':'bg-emerald-100 text-emerald-700') : (isDarkMode?'bg-red-900/50 text-red-400':'bg-red-100 text-red-700')}`}>{r.status}</span>
                      <button onClick={()=>deleteAttendanceRecord(r)} className={`p-1 rounded transition-colors ${isDarkMode ? 'text-slate-500 hover:text-red-400 hover:bg-slate-700' : 'text-gray-400 hover:text-red-500 hover:bg-gray-100'}`}><Trash2 size={14} /></button>
                    </div>
                  </div>
        ...
        <form onSubmit={handleSave} className="space-y-4 text-sm">
          {!editingId && (
            <div className={`flex p-1 rounded-xl mb-4 ${isDarkMode ? 'bg-slate-900' : 'bg-slate-100'}`}>
              ...
              <button type="button" onClick={()=>setFormData(p=>({...p,program:'mbk'}))} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${formData.program==='mbk'?(isDarkMode ? 'bg-slate-700 shadow-sm text-indigo-400' : 'bg-white shadow-sm text-indigo-600'):(isDarkMode ? 'text-slate-400 hover:text-slate-200' : 'text-slate-500 hover:text-slate-700')}`}>Murid MBK & OKU</button>
            </div>
          )}
          
          <div className={`flex flex-col items-center justify-center p-4 border-2 border-dashed rounded-2xl transition-colors ${isDarkMode ? 'bg-slate-800/50 border-slate-600 hover:border-indigo-500' : 'bg-slate-50 border-slate-200 hover:border-indigo-400'}`}>
            <div className="mb-3">
              <Avatar name={formData.name||'Student'} photoUrl={formData.photoUrl} size="w-20 h-20"/>
            </div>
            <label className="cursor-pointer">
              <span className={`flex items-center gap-2 text-sm font-bold transition-colors ${isDarkMode ? 'text-indigo-400 hover:text-indigo-300' : 'text-indigo-600 hover:text-indigo-700'}`}>
            ...
            {formData.photoUrl && (
              <div className="flex items-center gap-3 mt-2">
                <button type="button" onClick={()=>{setRawImageSrc(formData.photoUrl);setUploadType('profile');}} className={`text-xs font-bold hover:underline ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`}>Adjust</button>
                <span className={isDarkMode ? 'text-slate-600' : 'text-gray-300'}>|</span>
                <button type="button" onClick={()=>handleRemovePhoto('profile')} className={`text-xs font-bold hover:underline ${isDarkMode ? 'text-red-400' : 'text-red-500'}`}>Remove</button>
              </div>
            )}
            <p className={`text-xs mt-2 ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>Max size 5MB</p>
          </div>

          <div>
            <label className={`block text-sm font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Full Name</label>
            <input required placeholder="e.g. Jane Doe" className={`w-full p-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white placeholder-slate-400' : 'bg-white border-gray-300'}`} value={formData.name} onChange={e=>setFormData({...formData,name:e.target.value})}/>
          </div>
          
          <div>
            <label className={`block text-sm font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Jantina (Gender)</label>
            <div className={`flex gap-6 p-2.5 border rounded-xl ${isDarkMode ? 'border-slate-700 bg-slate-800/50' : 'border-slate-200 bg-white'}`}>
              <label className="flex items-center gap-2 cursor-pointer"><input type="radio" className="text-indigo-600 focus:ring-indigo-500 w-4 h-4" checked={formData.gender==='Lelaki'} onChange={()=>setFormData({...formData,gender:'Lelaki'})}/> <span className={`text-sm ${isDarkMode ? 'text-white' : 'text-slate-700'}`}>Lelaki</span></label>
              <label className="flex items-center gap-2 cursor-pointer"><input type="radio" className="text-indigo-600 focus:ring-indigo-500 w-4 h-4" checked={formData.gender==='Perempuan'} onChange={()=>setFormData({...formData,gender:'Perempuan'})}/> <span className={`text-sm ${isDarkMode ? 'text-white' : 'text-slate-700'}`}>Perempuan</span></label>
            </div>
          </div>
          ...
          {formData.program === 'pemulihan' ? (
            <>
              <div>
                <label className={`block text-sm font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>IC Number (Optional but Recommended)</label>
                <input placeholder="For Auto-Year (e.g. 16...)" className={`w-full p-2.5 border rounded-xl font-mono text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white placeholder-slate-400' : 'bg-white border-gray-300'}`} value={formData.ic} onChange={e=>setFormData({...formData,ic:e.target.value.replace(/\D/g,'')})} maxLength={12}/>
              </div>
              <div>
              ...
              <div>
                <label className={`block text-sm font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Subject</label>
                <select className={`w-full p-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-gray-300'}`} value={formData.subject} onChange={e=>setFormData({...formData,subject:e.target.value})}>
                  {subjects.map(s=><option key={s} value={s}>{s}</option>)}
                </select>
              </div>
              <label className={`flex items-center gap-2 font-bold p-3 border rounded-xl cursor-pointer ${isDarkMode ? 'border-slate-700 bg-slate-800/50 text-slate-300' : 'border-slate-200 bg-white text-slate-700'}`}>
                <input type="checkbox" checked={formData.isNewStudent} onChange={e=>setFormData({...formData,isNewStudent:e.target.checked})} className="w-4 h-4 text-indigo-600 focus:ring-indigo-500 rounded border-gray-300"/> 
                Murid Baru (插班生)
              </label>
            </>
          ) : (
            <>
              <div>
                <label className={`block text-sm font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Kategori (Category)</label>
                <div className={`flex gap-6 p-2.5 border rounded-xl ${isDarkMode ? 'border-slate-700 bg-slate-800/50' : 'border-slate-200 bg-white'}`}>
                  <label className="flex items-center gap-2 cursor-pointer"><input type="radio" className="text-amber-600 focus:ring-amber-500 w-4 h-4" checked={formData.mbkType==='MBK'} onChange={()=>setFormData({...formData,mbkType:'MBK'})}/> <span className={`text-sm ${isDarkMode ? 'text-white' : 'text-slate-700'}`}>MBK (Tiada Kad)</span></label>
                  <label className="flex items-center gap-2 cursor-pointer"><input type="radio" className="text-emerald-600 focus:ring-emerald-500 w-4 h-4" checked={formData.mbkType==='OKU'} onChange={()=>setFormData({...formData,mbkType:'OKU'})}/> <span className={`text-sm ${isDarkMode ? 'text-white' : 'text-slate-700'}`}>OKU (Ada Kad)</span></label>
                </div>
              </div>
              <div>
                <label className={`block text-sm font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>MyKid / IC Number</label>
                <input required placeholder="e.g. 160520101234" className={`w-full p-2.5 border rounded-xl font-mono text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white placeholder-slate-400' : 'bg-white border-gray-300'}`} value={formData.ic} onChange={e=>setFormData({...formData,ic:e.target.value.replace(/\D/g,'')})} maxLength={12}/>
              ...
              <div>
                <label className={`block text-sm font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Remarks</label>
                <textarea placeholder="Enter optional remarks..." className={`w-full p-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white placeholder-slate-400' : 'bg-white border-gray-300'}`} rows="2" value={formData.remarks} onChange={e=>setFormData({...formData,remarks:e.target.value})}/>
              </div>
              <div>
                <label className={`block text-sm font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Document Link</label>
                <input placeholder="https://..." className={`w-full p-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white placeholder-slate-400' : 'bg-white border-gray-300'}`} value={formData.docLink} onChange={e=>setFormData({...formData,docLink:e.target.value})}/>
              </div>
              ...
              {formData.mbkType === 'OKU' && (
                <div className={`p-4 border rounded-xl flex items-center justify-between ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                  <span className={`font-bold flex items-center gap-2 text-sm ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}><QrCode size={16}/> Kad OKU QR</span>
                  <div className="flex items-center gap-3">
                    {formData.qrCodeUrl && <button type="button" onClick={()=>handleRemovePhoto('qr')} className="text-red-500 text-xs font-bold hover:underline">Remove</button>}
                    <label className={`px-4 py-2 rounded-lg cursor-pointer font-bold text-xs transition-colors ${isDarkMode ? 'bg-indigo-900/50 text-indigo-400 hover:bg-indigo-800' : 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200'}`}>
                      Upload
          ...
          <div className="flex gap-3 pt-4">
            <button type="button" onClick={()=>setIsModalOpen(false)} className={`flex-1 py-3 text-sm font-bold rounded-xl transition-colors ${isDarkMode ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>Cancel</button>
            <button type="submit" className="flex-1 py-3 text-sm text-white font-bold rounded-xl shadow-sm bg-indigo-600 hover:bg-indigo-700 transition-colors">{editingId ? 'Save Changes' : 'Add Student'}</button>
          </div>
        </form>
      </Modal>
    </div>
