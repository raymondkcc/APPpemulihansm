    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col border border-slate-200 dark:border-slate-700">
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-700 flex gap-3">
          ...
          <button onClick={handleSave} className="flex-1 py-2.5 font-bold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow-md shadow-indigo-200 dark:shadow-indigo-900/20">Save</button>
        </div>
...
const Avatar = ({ name, color, photoUrl, size = "w-12 h-12", onClick }) => {
  ...
  if (photoUrl) {
    return <img src={photoUrl} alt={name} className={`${commonClasses} object-cover object-top bg-white dark:bg-slate-800`} onClick={onClick} />;
  }
...
export default function StudentDatabaseApp() {
  ...
  const renderStudentCard = (student, sectionType) => {
    ...
    return (
      <div key={student.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm hover:shadow-lg border border-slate-200 dark:border-slate-700 transition-all duration-300 hover:-translate-y-1 group relative overflow-hidden flex flex-col">
        ...
        <div className={`hidden sm:flex flex-col items-center gap-4 ${isMbk ? 'p-6' : 'p-4'}`}>
          ...
          <div className="w-full text-center">
            <h3 className={`font-bold ${isMbk ? 'text-lg' : 'text-sm'} text-slate-900 dark:text-white leading-tight mb-1 break-words`}>{student.name}</h3>
            ...
            {isMbk ? (
               <>
                 ...
                 <div className="text-sm text-slate-600 dark:text-slate-400 font-medium">{student.gender} • <span className={`px-2 py-0.5 rounded text-xs font-bold ${student.mbkType === 'OKU' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400'}`}>{student.mbkType || 'MBK'}</span></div>
               </>
            ) : (
               <>
            ...
            {isProfile && (
              <div className="flex flex-col gap-1 w-full mt-1">
                <div className="flex justify-between items-center text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">
                  ...
                  <span className={stats.percent >= 75 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}>{stats.percent}%</span>
                </div>
                <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden"><div className={`h-full rounded-full ${stats.percent >= 75 ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${stats.percent}%` }}></div></div>
              </div>
            )}
          </div>
          
          {isMbk && student.remarks && <div className="w-full mt-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800/50 rounded-lg flex items-start gap-2 text-left"><MessageSquare size={16} className="text-yellow-600 dark:text-yellow-500 mt-0.5 flex-shrink-0" /><p className="text-xs text-slate-700 dark:text-slate-300 italic">{student.remarks}</p></div>}
          
          {isMbk && (
            <div className="mt-2 w-full pt-4 border-t border-slate-100 dark:border-slate-700 flex flex-col gap-2">
              <button onClick={() => window.open(student.docLink, '_blank')} disabled={!student.docLink} className={`flex-1 flex items-center justify-center gap-2 text-sm font-bold text-white py-2.5 rounded-xl transition-all shadow-sm ${student.docLink ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-slate-300 dark:bg-slate-700 cursor-not-allowed text-slate-500'}`}><FileText size={16} /> Docs</button>
              <button onClick={() => handleCheckOKU(student.ic)} className="flex-1 flex items-center justify-center gap-2 text-sm font-bold text-white bg-slate-900 hover:bg-slate-800 dark:bg-slate-700 dark:hover:bg-slate-600 py-2.5 rounded-xl transition-all shadow-sm"><Search size={16} /> Semakan OKU</button>
            </div>
          )}
          ...
          {role === 'admin' && (
            <div className={`absolute top-2 right-2 flex ${isMbk ? 'flex-row' : 'flex-col'} gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 dark:bg-slate-800/90 p-1 rounded-lg backdrop-blur-sm shadow-sm border border-slate-100 dark:border-slate-700`}>
               {(!isMbk && !isLulus) && <button onClick={() => openNotesModal(student)} className="p-1.5 text-amber-500 hover:bg-amber-50 dark:hover:bg-slate-700 rounded transition-colors" title="Notes"><StickyNote size={14} /></button>}
               {isProfile && <button onClick={() => openAttendanceModal(student)} className="p-1.5 text-blue-500 hover:bg-blue-50 dark:hover:bg-slate-700 rounded transition-colors" title="Attendance"><Calendar size={14} /></button>}
               <button onClick={() => openEdit(student)} className="p-1.5 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded transition-colors" title="Edit"><Edit2 size={14} /></button>
               {!isMbk && <button onClick={() => toggleStudentStatus(student)} className="p-1.5 text-purple-500 hover:bg-purple-50 dark:hover:bg-slate-700 rounded transition-colors" title="Change Status"><RotateCcw size={14} /></button>}
               <button onClick={() => confirmDelete(student)} className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-slate-700 rounded transition-colors" title="Delete"><Trash2 size={14} /></button>
            </div>
          )}
        </div>
        ...
        <div className="sm:hidden flex flex-row items-start p-3 gap-3 relative z-10">
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            {role === 'admin' && (
              <div className="grid grid-cols-2 gap-1 w-[70px]">
                 {(!isMbk && !isLulus) && <button onClick={() => openNotesModal(student)} className="p-1 text-amber-500 bg-amber-50 dark:bg-slate-700 dark:border-slate-600 rounded border border-amber-100 flex justify-center"><StickyNote size={12} /></button>}
                 {isProfile && <button onClick={() => openAttendanceModal(student)} className="p-1 text-blue-500 bg-blue-50 dark:bg-slate-700 dark:border-slate-600 rounded border border-blue-100 flex justify-center"><Calendar size={12} /></button>}
                 <button onClick={() => openEdit(student)} className={`p-1 text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-700 dark:border-slate-600 rounded border border-slate-100 flex justify-center ${isMbk || isLulus ? '' : 'col-span-2'}`}><Edit2 size={12} /></button>
                 {!isMbk && <button onClick={() => toggleStudentStatus(student)} className="p-1 text-purple-500 bg-purple-50 dark:bg-slate-700 dark:border-slate-600 rounded border border-purple-100 flex justify-center"><RotateCcw size={12} /></button>}
                 <button onClick={() => confirmDelete(student)} className="p-1 text-red-500 bg-red-50 dark:bg-slate-700 dark:border-slate-600 rounded border border-red-100 flex justify-center col-span-2"><Trash2 size={12} /></button>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 min-w-0 text-left">
            ...
            <div className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-0.5">{isMbk ? (year < 1 ? 'Pra' : `Tahun ${year}`) : student.className}</div>
            ...
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1 flex items-center gap-1">
               ...
               {isMbk && <span className={`px-1.5 py-0.5 rounded border text-[9px] ${student.mbkType === 'OKU' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800' : 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800'}`}>{student.mbkType || 'MBK'}</span>}
            </div>
            ...
            {isProfile && (
              <div className="flex flex-col gap-1 w-full mt-1">
                <div className="flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase"><span>Attendance</span><span className={stats.percent >= 75 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}>{stats.percent}%</span></div>
                <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden"><div className={`h-full rounded-full ${stats.percent >= 75 ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${stats.percent}%` }}></div></div>
              </div>
            )}

            {isMbk && student.remarks && <div className="text-[10px] text-slate-500 dark:text-slate-400 italic bg-yellow-50 dark:bg-yellow-900/20 px-2 py-1 rounded border border-yellow-100 dark:border-yellow-800/50 flex items-start gap-1 mt-1"><MessageSquare size={10} className="mt-0.5 flex-shrink-0" /><span className="line-clamp-2">{student.remarks}</span></div>}
            
            {isMbk && (
              <div className="mt-2 flex flex-col gap-1">
                 <button onClick={() => window.open(student.docLink, '_blank')} disabled={!student.docLink} className={`flex items-center justify-center gap-1 text-[10px] font-bold py-1 px-2 rounded border ${student.docLink ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border-indigo-100 dark:border-indigo-800' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 border-slate-100 dark:border-slate-700'}`}><FileText size={12} /> {student.docLink ? 'Docs' : 'No Docs'}</button>
                 <button onClick={() => handleCheckOKU(student.ic)} className="flex items-center justify-center gap-1 text-[10px] font-bold py-1 px-2 rounded border bg-slate-900 dark:bg-slate-700 text-white dark:border-slate-600"><Search size={12} /> Semakan OKU</button>
                 {student.qrCodeUrl && <button onClick={() => setFullScreenImage(student.qrCodeUrl)} className="flex items-center justify-center gap-1 text-[10px] font-bold py-1 px-2 rounded border bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-800"><QrCode size={12} /> QR Code</button>}
              </div>
            )}
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-indigo-100 dark:selection:bg-indigo-900 transition-colors duration-300 ${isDarkMode ? 'dark bg-slate-900 text-slate-100' : 'bg-gradient-to-br from-slate-50 to-slate-100 text-slate-800'}`}>
          ...
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-lg tracking-tight hidden lg:block">
                Pengurusan Program Pemulihan Digital <span className="text-indigo-600 dark:text-indigo-400">SJKC SIN MING, PUCHONG</span>
              </span>
              <span className="font-bold text-lg tracking-tight hidden sm:block lg:hidden">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-32">
        ...
        {currentSection === 'stats' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className={`p-6 rounded-2xl border shadow-sm transition-colors ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
              <h3 className={`text-lg font-bold mb-4 flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}><Filter size={20} className="text-indigo-500" /> Find Database</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="relative">
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Year</label>
                  <select className={`w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500 appearance-none font-medium transition-colors ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={statsFilters.year} onChange={(e) => setStatsFilters(p => ({...p, year: e.target.value}))}>
                ...
                <div className="relative">
                  ...
                  <select className={`w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500 appearance-none font-medium transition-colors ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={statsFilters.gender} onChange={(e) => setStatsFilters(p => ({...p, gender: e.target.value}))}>
                ...
                <div className="relative">
                  ...
                  <select className={`w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500 appearance-none font-medium transition-colors ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={statsFilters.subject} onChange={(e) => setStatsFilters(p => ({...p, subject: e.target.value}))}>
            ...
            <div className={`p-8 rounded-2xl border flex items-center justify-between shadow-sm transition-colors ${isDarkMode ? 'bg-orange-900/20 border-orange-800/50' : 'bg-orange-50 border-orange-100'}`}>
              <div>
                 <p className={`text-sm font-bold uppercase ${isDarkMode ? 'text-orange-400' : 'text-orange-600'}`}>Students Found (Pemulihan Only)</p>
                 <h2 className={`text-5xl font-extrabold mt-1 ${isDarkMode ? 'text-orange-300' : 'text-orange-900'}`}>{filteredStudents.length}</h2>
              </div>
                ...
                <div className="overflow-x-auto w-full">
                  <table className="w-full text-left text-sm whitespace-nowrap">
                    <thead className={`border-b ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-100'}`}>
                      <tr>
                        <th className={`px-6 py-4 font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Name</th>
                        <th className={`px-6 py-4 font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Gender</th>
                        <th className={`px-6 py-4 font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Class</th>
                        <th className={`px-6 py-4 font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Subject</th>
                      </tr>
                    </thead>
                    <tbody className={`divide-y ${isDarkMode ? 'divide-slate-700' : 'divide-slate-50'}`}>
                      {filteredStudents.map(student => (
                        <tr key={student.id} className={`transition-colors ${isDarkMode ? 'hover:bg-slate-700/50' : 'hover:bg-slate-50/80'}`}>
                          <td className={`px-6 py-4 font-bold ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{student.name}</td>
                          <td className={`px-6 py-4 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>{student.gender || 'Lelaki'}</td>
                          ...
                          <td className={`px-6 py-4 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>{student.subject}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                 ...
                 <div className={`border-b p-6 flex flex-col md:flex-row justify-between items-center gap-4 transition-colors ${isDarkMode ? 'bg-slate-900/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className={`inline-block px-2 py-0.5 rounded text-xs font-bold text-white shadow-sm ${getSubjectBadgeColor(selectedStudentForProgress.subject)}`}>{selectedStudentForProgress.subject}</div>
                      </div>
                    </div>
                 ...
                 <div className="p-6 md:p-8">
                    <div className="mb-8">
                      <div className="flex justify-between items-end mb-2">
                        ...
                        <span className={`text-sm font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>{studentProgressData[progressSubject === 'BM' ? 'bm' : 'math']?.length || 0} / {progressSubject === 'BM' ? KEMAHIRAN_BM.length : KEMAHIRAN_MATH.length}</span>
                      </div>
                  ...
                  <div className="max-w-2xl mx-auto">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full mb-4">
                        <div className="relative group flex-1"><select className={`w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500 font-bold text-sm appearance-none transition-colors ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={profileYearFilter} onChange={(e) => setProfileYearFilter(e.target.value)}>
                          <option value="All">Filter: All Years</option>
                          {availableYears.filter(y => y !== 'All').map(y => <option key={y} value={y}>{`Tahun ${y}`}</option>)}
                        </select><ChevronDown className="absolute right-3 top-3 text-slate-400 w-4 h-4 pointer-events-none" /></div>
                        <div className="relative group flex-1"><select className={`w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500 font-bold text-sm appearance-none transition-colors ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={classFilter} onChange={(e) => setClassFilter(e.target.value)}>
                          <option value="All">Filter: All Classes</option>
                          {availableClasses.filter(c => c !== 'All').map(c => <option key={c} value={c}>{c}</option>)}
                        </select><ChevronDown className="absolute right-3 top-3 text-slate-400 w-4 h-4 pointer-events-none" /></div>
                        <div className="relative group flex-1"><select className={`w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500 font-bold text-sm appearance-none transition-colors ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={subjectFilter} onChange={(e) => setSubjectFilter(e.target.value)}>
                          <option value="All">Filter: All Subjects</option>
                          {subjects.map(s => <option key={s} value={s}>{s}</option>)}
                        </select><ChevronDown className="absolute right-3 top-3 text-slate-400 w-4 h-4 pointer-events-none" /></div>
                    </div>
                    <div className="relative mb-6">
                      ...
                      <input type="text" placeholder="Search student name..." className={`w-full pl-12 pr-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium transition-colors ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white placeholder-slate-400' : 'bg-slate-50 border-slate-200 text-slate-900'}`} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                    <div className="space-y-2 max-h-[400px] overflow-y-auto">
                       {filteredStudents.map(student => (
                         <button key={student.id} onClick={() => { setSelectedStudentForProgress(student); setStudentProgressData(student.progress || { bm: [], math: [] }); setProgressSubject((student.subject || '').includes('Matematik') && !(student.subject || '').includes('BM') ? 'MATH' : 'BM'); }} className={`w-full flex items-center gap-3 p-3 border rounded-xl transition-all text-left group ${isDarkMode ? 'bg-slate-800 border-slate-700 hover:border-indigo-500' : 'bg-white border-slate-100 hover:border-indigo-200 hover:shadow-md'}`}>
                            ...
                            <div><h4 className={`font-bold transition-colors ${isDarkMode ? 'text-white group-hover:text-indigo-400' : 'text-slate-800 group-hover:text-indigo-700'}`}>{student.name}</h4><p className="text-xs text-slate-500">{student.className}</p></div>
        ...
        {(['profile', 'plan', 'mbk', 'lulus'].includes(currentSection)) && (
           <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
             ...
             <div className={`flex flex-col lg:flex-row gap-4 mb-8 justify-between items-start lg:items-center p-4 rounded-2xl shadow-sm border transition-colors ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
                {currentSection === 'profile' && (
                  <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                    <div className="relative group"><select className={`w-full sm:w-48 px-4 py-2.5 border rounded-xl font-bold text-sm appearance-none transition-colors focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={profileYearFilter} onChange={(e) => setProfileYearFilter(e.target.value)}><option value="All">Filter: All Years</option>{availableYears.filter(y=>y!=='All').map(y => <option key={y} value={y}>{`Tahun ${y}`}</option>)}</select><ChevronDown className="absolute right-3 top-3 text-slate-400 w-4 h-4 pointer-events-none" /></div>
                    <div className="relative group"><select className={`w-full sm:w-48 px-4 py-2.5 border rounded-xl font-bold text-sm appearance-none transition-colors focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={classFilter} onChange={(e) => setClassFilter(e.target.value)}><option value="All">Filter: All Classes</option>{availableClasses.filter(c=>c!=='All').map(c => <option key={c} value={c}>{c}</option>)}</select><ChevronDown className="absolute right-3 top-3 text-slate-400 w-4 h-4 pointer-events-none" /></div>
                    <div className="relative group"><select className={`w-full sm:w-48 px-4 py-2.5 border rounded-xl font-bold text-sm appearance-none transition-colors focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`} value={subjectFilter} onChange={(e) => setSubjectFilter(e.target.value)}><option value="All">Filter: All Subjects</option>{subjects.map(s => <option key={s} value={s}>{s}</option>)}</select><ChevronDown className="absolute right-3 top-3 text-slate-400 w-4 h-4 pointer-events-none" /></div>
                  </div>
                )}
                ...
                {currentSection === 'mbk' && <h2 className={`text-2xl font-extrabold tracking-tight ml-4 ${isDarkMode ? 'text-indigo-400' : 'text-indigo-900'}`}>Senarai Murid MBK & OKU</h2>}
             ...
             {/* Dynamic Grids */}
             {loading ? <div className="text-center py-24"><div className="animate-spin rounded-full h-12 w-12 border-4 border-blue-100 border-t-blue-600 mx-auto mb-4"></div><p className="text-slate-400 font-medium">Loading database...</p></div> : 
               ...
               <div className={`text-center py-24 rounded-3xl border border-dashed shadow-sm ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300'}`}><Users className="text-slate-400 w-10 h-10 mx-auto mb-4"/><h3 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>No students found</h3></div> : 
               ...
               <div className="space-y-10">
                  ...
                  {currentSection === 'plan' && Object.keys(groupedPlanStudents).sort().map(yearGrp => (
                    <div key={yearGrp} className={`rounded-3xl border shadow-sm overflow-hidden ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
      ...
      <Modal isOpen={showAdminLogin} onClose={()=>{setShowAdminLogin(false); setLoginError('');}} title="Admin Login">
        <div className="flex flex-col items-center mb-6"><div className="bg-blue-50 dark:bg-blue-900/30 p-3 rounded-full mb-3"><Lock className="text-blue-600 dark:text-blue-400 w-8 h-8" /></div><p className="text-sm text-gray-500 dark:text-gray-400">Enter admin password.</p></div>
        <form onSubmit={handleAdminLogin} className="space-y-4">
          <input type="password" placeholder="Password" autoFocus className={`w-full p-2 border rounded focus:ring-2 focus:ring-blue-500 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} value={adminPassword} onChange={e=>{setAdminPassword(e.target.value); setLoginError('');}}/>
      ...
      <Modal isOpen={deleteConfirmation.isOpen} onClose={()=>setDeleteConfirmation({isOpen:false})} title="Confirm Deletion">
        <div className="flex flex-col items-center mb-6"><div className="bg-red-50 dark:bg-red-900/30 p-3 rounded-full mb-3"><Trash2 className="text-red-600 dark:text-red-400 w-8 h-8" /></div><p className="dark:text-slate-300">Delete <span className="font-bold dark:text-white">{deleteConfirmation.studentName}</span>?</p></div>
        <div className="flex gap-3"><button onClick={()=>setDeleteConfirmation({isOpen:false})} className={`flex-1 py-2 font-bold rounded ${isDarkMode ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>Cancel</button><button onClick={executeDelete} className="flex-1 py-2 bg-red-600 text-white font-bold rounded hover:bg-red-700">Delete</button></div>
      </Modal>
      ...
      <Modal isOpen={moveConfirmation.isOpen} onClose={()=>setMoveConfirmation({isOpen:false})} title="Change Status">
        <div className="flex flex-col items-center mb-6"><div className="bg-blue-50 dark:bg-blue-900/30 p-3 rounded-full mb-3"><ArrowLeftRight className="text-blue-600 dark:text-blue-400 w-8 h-8" /></div><p className="mb-4 dark:text-slate-300">Change status to: <span className="font-bold dark:text-white">{moveConfirmation.newStatus}</span>?</p>
        {moveConfirmation.newStatus === 'Lulus' && <div className="w-full text-left"><label className="text-xs font-bold text-slate-500">Graduation Date</label><input type="date" value={moveDate} onChange={e=>setMoveDate(e.target.value)} className={`w-full mt-1 p-2 border rounded ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`}/></div>}</div>
        <div className="flex gap-3"><button onClick={()=>setMoveConfirmation({isOpen:false})} className={`flex-1 py-2 font-bold rounded ${isDarkMode ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>Cancel</button><button onClick={executeMove} className="flex-1 py-2 bg-blue-600 text-white font-bold rounded hover:bg-blue-700">Confirm</button></div>
      </Modal>

      {/* Notes Modal */}
      <Modal isOpen={isNotesModalOpen} onClose={()=>setIsNotesModalOpen(false)} title="Catatan">
        <div className="flex gap-3 mb-4"><Avatar name={selectedStudentForNotes?.name||''} photoUrl={selectedStudentForNotes?.photoUrl}/><div className="font-bold mt-1 dark:text-white">{selectedStudentForNotes?.name}</div></div>
        <form onSubmit={saveNote} className="space-y-3">
          <input type="date" required className={`w-full p-2 border rounded focus:ring-2 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} value={noteForm.date} onChange={e=>setNoteForm(p=>({...p, date:e.target.value}))}/>
          <textarea required rows="3" placeholder="Notes here..." className={`w-full p-2 border rounded focus:ring-2 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} value={noteForm.text} onChange={e=>setNoteForm(p=>({...p, text:e.target.value}))}/>
        ...
        <div className="mt-4 max-h-40 overflow-auto space-y-2">
          {[...(selectedStudentForNotes?.notes || [])].sort((a,b)=>new Date(b.date)-new Date(a.date)).map(n=>(
            <div key={n.id} className={`p-3 border rounded text-sm group relative ${isDarkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-200'}`}>
              <span className={`font-bold px-2 py-0.5 rounded border mr-2 ${isDarkMode ? 'bg-slate-800 text-slate-300 border-slate-600' : 'bg-white text-slate-500 border-slate-200'}`}>{n.date}</span>
              <p className={`mt-2 whitespace-pre-wrap ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>{n.text}</p>
              <div className={`absolute top-2 right-2 hidden group-hover:flex gap-1 p-1 shadow-sm rounded ${isDarkMode ? 'bg-slate-800' : 'bg-white/90'}`}>
                <button onClick={()=>startEditNote(n)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded"><Edit2 size={14} className="text-blue-500"/></button>
                <button onClick={()=>deleteNote(n.id)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded"><Trash2 size={14} className="text-red-500"/></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal isOpen={isAttendanceModalOpen} onClose={()=>setIsAttendanceModalOpen(false)} title="Attendance">
         <div className="flex gap-3 mb-4"><Avatar name={selectedStudentForAttendance?.name||''} photoUrl={selectedStudentForAttendance?.photoUrl}/><div className="font-bold mt-1 dark:text-white">{selectedStudentForAttendance?.name}</div></div>
         <input type="date" className={`w-full p-2 border rounded mb-3 focus:ring-2 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} value={attendanceDate} onChange={e=>setAttendanceDate(e.target.value)}/>
         <div className="flex gap-2 mb-4">
           <button onClick={()=>markAttendance('present')} className="flex-1 flex items-center justify-center gap-1 py-2 bg-emerald-600 text-white font-bold rounded hover:bg-emerald-700"><Check size={16}/> Present</button>
           <button onClick={()=>markAttendance('absent')} className="flex-1 flex items-center justify-center gap-1 py-2 bg-red-600 text-white font-bold rounded hover:bg-red-700"><X size={16}/> Absent</button>
         </div>
         <div className={`max-h-40 overflow-auto divide-y ${isDarkMode ? 'divide-slate-700' : 'divide-slate-100'}`}>
            {[...(selectedStudentForAttendance?.attendanceRecords || [])].sort((a,b)=>new Date(b.date)-new Date(a.date)).map((r,i)=>(
              <div key={i} className={`py-2 flex justify-between items-center ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                <span className="font-medium text-sm">{r.date}</span>
                <div className="flex gap-3 items-center">
                  <span className={`text-xs font-bold px-2 py-0.5 rounded uppercase ${r.status==='present'?'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-400':'bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-400'}`}>{r.status}</span>
                  <button onClick={()=>deleteAttendanceRecord(r)} className="hover:bg-slate-100 dark:hover:bg-slate-700 p-1 rounded"><Trash2 size={14} className="text-slate-400 hover:text-red-500"/></button>
                </div>
              </div>
            ))}
         </div>
        ...
        <form onSubmit={handleSave} className="space-y-4 text-sm">
          {!editingId && (
            <div className={`flex p-1 rounded-lg ${isDarkMode ? 'bg-slate-900' : 'bg-slate-100'}`}>
              <button type="button" onClick={()=>setFormData(p=>({...p,program:'pemulihan'}))} className={`flex-1 py-1.5 rounded transition-all ${formData.program==='pemulihan' ? (isDarkMode ? 'bg-slate-700 font-bold shadow-sm text-blue-400' : 'bg-white font-bold shadow-sm text-blue-600') : (isDarkMode ? 'text-slate-400' : 'text-slate-500')}`}>Pemulihan</button>
              <button type="button" onClick={()=>setFormData(p=>({...p,program:'mbk'}))} className={`flex-1 py-1.5 rounded transition-all ${formData.program==='mbk' ? (isDarkMode ? 'bg-slate-700 font-bold shadow-sm text-indigo-400' : 'bg-white font-bold shadow-sm text-indigo-600') : (isDarkMode ? 'text-slate-400' : 'text-slate-500')}`}>MBK & OKU</button>
            </div>
          )}
          
          <div className={`flex flex-col items-center p-3 border-2 border-dashed rounded-xl ${isDarkMode ? 'bg-slate-900/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
            <Avatar name={formData.name||'Student'} photoUrl={formData.photoUrl} size="w-16 h-16"/>
            <label className="text-blue-600 dark:text-blue-400 font-bold cursor-pointer mt-2 flex items-center gap-1"><Camera size={14}/> {formData.photoUrl?'Change':'Upload'} Photo<input type="file" hidden accept="image/*" onChange={e=>handleImageUpload(e,'profile')}/></label>
          ...
          <div><label className={`block font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Full Name</label><input required placeholder="Full Name" className={`w-full p-2.5 border rounded-xl focus:ring-2 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} value={formData.name} onChange={e=>setFormData({...formData,name:e.target.value})}/></div>
          
          <div>
            <label className={`block font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Gender</label>
            <div className="flex gap-6"><label className={`flex items-center gap-2 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}><input type="radio" checked={formData.gender==='Lelaki'} onChange={()=>setFormData({...formData,gender:'Lelaki'})}/> Lelaki</label><label className={`flex items-center gap-2 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}><input type="radio" checked={formData.gender==='Perempuan'} onChange={()=>setFormData({...formData,gender:'Perempuan'})}/> Perempuan</label></div>
          </div>

          {formData.program === 'pemulihan' ? (
            <>
              <div><label className={`block font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>IC Number (for auto-year)</label><input placeholder="e.g. 1605..." className={`w-full p-2.5 border rounded-xl font-mono focus:ring-2 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} value={formData.ic} onChange={e=>setFormData({...formData,ic:e.target.value.replace(/\D/g,'')})} maxLength={12}/></div>
              <div><label className={`block font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Class</label><input required placeholder="e.g. 2 Hebat" className={`w-full p-2.5 border rounded-xl font-mono focus:ring-2 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} value={formData.className} onChange={handleClassNameChange}/></div>
              <div><label className={`block font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Subject</label><select className={`w-full p-2.5 border rounded-xl focus:ring-2 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} value={formData.subject} onChange={e=>setFormData({...formData,subject:e.target.value})}>{subjects.map(s=><option key={s} value={s}>{s}</option>)}</select></div>
              <label className={`flex items-center gap-2 font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}><input type="checkbox" checked={formData.isNewStudent} onChange={e=>setFormData({...formData,isNewStudent:e.target.checked})} className="w-4 h-4"/> Murid Baru (插班生)</label>
            </>
          ) : (
            <>
              <div><label className={`block font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Category</label><div className="flex gap-6"><label className={`flex items-center gap-2 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}><input type="radio" checked={formData.mbkType==='MBK'} onChange={()=>setFormData({...formData,mbkType:'MBK'})}/> MBK (Tiada Kad)</label><label className={`flex items-center gap-2 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}><input type="radio" checked={formData.mbkType==='OKU'} onChange={()=>setFormData({...formData,mbkType:'OKU'})}/> OKU (Ada Kad)</label></div></div>
              <div><label className={`block font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>MyKid / IC</label><input required placeholder="e.g. 1605..." className={`w-full p-2.5 border rounded-xl font-mono focus:ring-2 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} value={formData.ic} onChange={e=>setFormData({...formData,ic:e.target.value.replace(/\D/g,'')})} maxLength={12}/></div>
              <div><label className={`block font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Remarks</label><textarea placeholder="Remarks..." className={`w-full p-2.5 border rounded-xl focus:ring-2 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} rows="2" value={formData.remarks} onChange={e=>setFormData({...formData,remarks:e.target.value})}/></div>
              <div><label className={`block font-bold mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Doc Link</label><input placeholder="https://..." className={`w-full p-2.5 border rounded-xl focus:ring-2 outline-none ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'}`} value={formData.docLink} onChange={e=>setFormData({...formData,docLink:e.target.value})}/></div>
              
              {formData.mbkType === 'OKU' && (
                <div className={`p-4 border rounded-xl flex items-center justify-between ${isDarkMode ? 'bg-slate-900/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                  <span className={`font-bold flex items-center gap-2 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}><QrCode size={16}/> Kad OKU QR</span>
                  <div className="flex items-center gap-3">
                    ...
                    <label className={`px-3 py-1.5 rounded-lg cursor-pointer font-bold text-xs transition-colors ${isDarkMode ? 'bg-indigo-900/50 text-indigo-400 hover:bg-indigo-900' : 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200'}`}>Upload<input type="file" hidden accept="image/*" onChange={e=>handleImageUpload(e,'qr')}/></label>
                  </div>
                </div>
              )}
            </>
          )}
          <div className="flex gap-3 pt-4"><button type="button" onClick={()=>setIsModalOpen(false)} className={`flex-1 py-3 font-bold rounded-xl transition-colors ${isDarkMode ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>Cancel</button><button type="submit" className={`flex-1 py-3 text-white font-bold rounded-xl shadow-sm ${formData.program==='mbk'?'bg-indigo-600 hover:bg-indigo-700':'bg-blue-600 hover:bg-blue-700'}`}>Save</button></div>
        </form>
      </Modal>
    </div>
  );
